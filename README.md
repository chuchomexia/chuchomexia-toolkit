...
Próximo paso natural: pilotar `init` en un proyecto real (no Colsanitas) antes de darla por confiable.
...
Este repo está pensado para seguir creciendo. Para sumar una nueva skill, en ambas herramientas a la vez:
...
Esto significa que **cada release toca dos archivos de versión** (`.claude-plugin/plugin.json` no necesita bump si se omite `version`, pero `.codex-plugin/plugin.json` sí). Fácil de olvidar — vale la pena revisar antes de cada push si cambió algo sustancial en el `skills/` de cualquiera de los plugins.
